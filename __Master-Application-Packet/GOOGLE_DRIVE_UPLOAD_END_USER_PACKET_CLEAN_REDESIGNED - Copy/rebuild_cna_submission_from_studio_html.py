...
def normalize_studio_page(page_html: str, footer_left: str) -> str:
    ...
    html = html.replace("Yadvir Saandal", "TJ Padilla")
    html = html.replace("YADVIR SAANDAL", "TJ PADILLA")
    html = re.sub(r"(?i)\bVisual placeholder\b", "Visual template", html)
    html = re.sub(r"(?i)24 HOUR CNA COURSE MATRIX FINAL\.md", "CNA course matrix", html)
    html = re.sub(r"(?i)\bOf\s+cial\b", "Official", html)
    html = re.sub(r"(?i)\bO\s+cial\b", "Official", html)
    html = re.sub(r"(?i)\bof\s+icial\b", "official", html)
    html = re.sub(r"(?i)\bconf\s+identiality\b", "confidentiality", html)
    html = re.sub(r"(?i)\b99_NOT_FOR_SUBMISSION_REFERENCE\b", "internal reference archive", html)
...
def minimum_scope_page() -> str:
    ...
    <div id="p2" class="page">
      ...
      {page_footer("TJ Padilla / CNA CDPH Submission Packet", "Page 2 of 8", "Prepared June 09, 2026")}
    </div>
    """
...
def review_context_page() -> str:
    ...
    <div id="p2" class="page">
      ...
      {page_footer("TJ Padilla / CNA CDPH Review Context", "Page 2 of 2", "Prepared June 09, 2026")}
    </div>
    """
...
def html_document(style: str, title: str, pages: list[str]) -> str:
    extra_css = """
      ...
      .doc-section-title::after {
        ...
        background: linear-gradient(90deg, var(--ci-gold) 0%, rgba(255,193,7,.72) 55%, transparent 100%) !important;
      }
      .intro-text {
...
def build() -> dict[str, object]:
    ...
    for page_num in range(2, 12):
        ...
        p2_to_p11.append(normalize_studio_page(str(page), "TJ Padilla / CNA CDPH Application Binder"))
