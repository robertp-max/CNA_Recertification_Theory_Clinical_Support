...
def clean_text(value: object) -> str:
    ...
    text = re.sub(r"(?i)\bTJ\s+Padilla\b", "Yadvir Saandal", text)
    text = re.sub(r"(?i)\bTJ PADILLA\b", "YADVIR SAANDAL", text)
...
def main() -> None:
    ...
    official_checks = verify_official_appends()
    ...
    report = {
        ...
        "qa_samples": render_samples(results),
    }
    REPORT.write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(json.dumps({"updated": len(results), "official_compare_failures": 0, "report": str(REPORT)}, separators=(",", ":")))
