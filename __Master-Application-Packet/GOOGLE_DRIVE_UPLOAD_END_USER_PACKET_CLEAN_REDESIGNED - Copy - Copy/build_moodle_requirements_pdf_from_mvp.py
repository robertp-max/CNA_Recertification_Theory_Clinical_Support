...
def clean_presentation_text(text: str) -> str:
    replacements = [
        (r"(?i)\bgenerated_missing_elements\b", "additional_elements"),
        (r"(?i)\bREADME_GENERATED_MISSING_ELEMENTS\.md\b", "README_ADDITIONAL_ELEMENTS.md"),
        (r"(?i)\b98_MISSING_OR_HUMAN_VERIFICATION_ITEMS\.md\b", "98_VERIFICATION_ITEMS.md"),
...
def html_document(document_part: str = "full") -> str:
  ...
  <div class="print-fixed-footer">
    <span class="footer-left">Yadvir Saandal / Implementation Requirement Binder</span>
...
def rel_label(path: Path) -> str:
    ...
    label = re.sub(r"(?i)generated_missing_elements", "additional_elements", label)
    label = re.sub(r"(?i)README_GENERATED_MISSING_ELEMENTS", "README_ADDITIONAL_ELEMENTS", label)
    label = re.sub(r"(?i)MISSING_OR_HUMAN_VERIFICATION_ITEMS", "VERIFICATION_ITEMS", label)
    label = re.sub(r"(?i)GENERATED_", "", label)
    label = re.sub(r"(?i)MISSING", "ADDITIONAL", label)
    return label
