...
- Use the role set defined in `04_ROLE_ACCESS_AND_WORKFLOW_MATRIX.md` and `import_assets/ROLE_MATRIX_IMPORT_REFERENCE.csv`, including a custom read-only External Survey Reviewer role and a custom Onboarding Policy and Workflow Viewer role.
...
- Onboarding accounts receive access only to the approved policy and workflow resource category. They must not receive course enrollment, learner records, grade/report access, certificate controls, export rights, reviewer access, teacher roles, manager roles, or access to course build materials.
