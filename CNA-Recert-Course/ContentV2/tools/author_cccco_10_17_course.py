...
Build the canonical ContentV2 course from CCCCO/NATP Modules 10-17.
...
Orientation/compliance is the only non-CCCCO instructional exception. This tool
...
def key_terms_from_terms(terms: list[str], fallback_title: str) -> list[dict]:
    ...
    for term in terms[:6]:
        ...
        out.append({
            "term": clean[:60],
            "definition": f"CCCCO source term used in this module; review in context for {fallback_title}.",
        })
    if not out:
        out = [{"term": fallback_title[:60], "definition": "Core CCCCO source concept for this lesson."}]
    return out
...
def quality_meta(module_n: int, obj_n: int, topic: str, status: str, value: str, not_dup: str, source_ref: str) -> dict:
    return {
        ...
        "transformation_type": "CCCCO-source transformation",
        "why_this_card_exists": f"Supports CCCCO Module {module_n} Objective {obj_n} without relying on non-CCCCO instructional content.",
...
def card(module_id: str, module_title: str, lesson: dict, card_id: str, card_type: str, title: str, text: str,
    ...
    return {
        ...
        "why_it_matters": [
            ...
            "Maps required theory to CCCCO Module 10-17 objectives.",
...
def module_assessment_question(module_n: int, obj_n: int, objective: str, title: str, idx: int) -> dict:
    ...
    return {
        ...
        "prompt": f"Which statement best matches CCCCO Module {module_n} Objective {obj_n}: {objective}?",
        "question_type": "Multiple choice",
        "choices": [
            {"id": "A", "label": f"Use the CCCCO Module {module_n} source concept while staying within CNA scope."},
        ...
        "difficulty": "Medium",
        "source_reference": f"CCCCO Module {module_n}, Objective {obj_n}",
        "sme_review_flag": "None identified",
...
def build_challenge(module_n: int, obj_n: int, objective: str) -> dict:
    return {
        "id": f"M{module_n}-O{obj_n:02d}-PRACTICE",
        "app_location": f"module.m{module_n}.lesson.l{obj_n:02d}.card.c90_challenge",
        "prompt": f"Practice check: For CCCCO Module {module_n} Objective {obj_n}, what is the safest CNA use of this theory?",
        "question_type": "Multiple choice",
        "choices": [
            {"id": "A", "label": "Observe the resident, follow the care plan, report changes, and document according to facility policy."},
            {"id": "B", "label": "Use the online lesson as permission to perform an unassigned hands-on skill."},
            {"id": "C", "label": "Delay reporting until the end of the shift if the resident seems stable."},
            {"id": "D", "label": "Tell the resident or family that the CNA has diagnosed the problem."},
        ],
        "correct_id_internal": "A",
        "rationale_internal": f"The safest response keeps CCCCO Module {module_n} Objective {obj_n} inside CNA scope: observe, assist as directed, report, document, and escalate.",
        "difficulty": "Medium",
        "source_reference": f"CCCCO Module {module_n}, Objective {obj_n}: {objective}",
        "sme_review_flag": "None identified",
...
def build_remediation(challenge: dict, module_n: int, obj_n: int) -> dict:
    return {
        ...
        "safest_response": {"option_id": "A", "text": challenge["choices"][0]["label"]},
        "selected_response": None,
        "why_correct": challenge["rationale_internal"],
        "why_incorrect": {
            "B": "Online theory does not validate hands-on clinical competency.",
            "C": "Delay can miss a change in condition or safety risk.",
            "D": "Diagnosis and care-plan changes are outside CNA scope.",
        },
        "option_rationales": [
            {"id": "A", "label": challenge["choices"][0]["label"], "status": "safest", "why": challenge["rationale_internal"], "remember": "Observe, report, document, and follow the care plan."},
            {"id": "B", "label": challenge["choices"][1]["label"], "status": "needs-review", "why": "Hands-on competency is not validated by this course.", "remember": "Only perform trained, assigned tasks."},
            {"id": "C", "label": challenge["choices"][2]["label"], "status": "needs-review", "why": "Important changes should be reported promptly.", "remember": "When in doubt, escalate."},
            {"id": "D", "label": challenge["choices"][3]["label"], "status": "needs-review", "why": "The CNA does not diagnose.", "remember": "Report observations to licensed staff."},
        ],
        ...
        "remember_this": f"CCCCO Module {module_n} Objective {obj_n} is theory support, not independent clinical authority.",
        "remediation_narration": f"Review CCCCO Module {module_n} Objective {obj_n}. The safest response stays within CNA scope and protects the resident.",
        "remediation_transcript": f"Review CCCCO Module {module_n} Objective {obj_n}. The safest response stays within CNA scope and protects the resident.",
        "remediation_media_prompt": media(f"module.m{module_n}.lesson.l{obj_n:02d}.remediation", f"CCCCO Module {module_n} Objective {obj_n} remediation"),
...
def add_source_backed_activity_cards(
    ...
    templates = [
        (
            "Source Term Sorting",
            f"Active learning task: use the CCCCO Module {module_n} terms for this objective ({term_names}) and sort them into three mental buckets: what you observe, what you report, and what you document. Keep the work de-identified. This strengthens retrieval of the source vocabulary without adding non-CCCCO content.",
        ),
        (
        ...
        (
            ...
            f"Active learning task: for CCCCO Module {module_n} Objective {lesson['lesson_id'][1:]}, decide which parts belong to online theory and which parts would require hands-on training or facility-specific direction. This reinforces the course boundary that online theory does not validate clinical competency.",
        ),
        (
        ...
        (
            "Quick Teach-Back",
            f"Active learning task: teach this objective back in two plain-language sentences: first, what the CCCCO source requires the learner to know; second, how the CNA applies it safely through observation, assistance as directed, reporting, documentation, and escalation.",
        ),
    ]
    source_ref = f"CCCCO Module {module_n}, Objective {int(lesson['lesson_id'][1:])}, source-backed activity"
    idx = 1
    while (current < target or (lesson_minutes >= 15 and delivery_count < 6)) and idx <= len(templates):
        ...
        activity_card["source_backed_activity_minutes"] = 2.0
...
def build_lesson(module_n: int, module_title: str, objective: dict, lesson_minutes: int, terms: list[str], section_lines: list[str]) -> dict:
    obj_n = objective["n"]
    ...
    lesson = {
        ...
        "status": "draft",
        "source_reference": f"CCCCO Module {module_n}, Objective {obj_n}",
        "learning_goal": f"Explain CCCCO Module {module_n} Objective {obj_n}: {objective_text}.",
    }
    status = "Deferred" if (module_n, obj_n) in DEFERRED_OBJECTIVES else "Covered"
    source_ref = f"CCCCO Module {module_n}, Objective {obj_n}"
    key_terms = key_terms_from_terms(terms, lesson["lesson_title"])
    overview_text = (
        f"CCCCO Module {module_n} Objective {obj_n} focuses on: {objective_text}. "
        f"This lesson uses the CCCCO source as the instructional authority. "
    ...
    for i, chunk_text in enumerate(chunks, start=1):
        ...
        text = (
            f"Source-supported teaching from CCCCO Module {module_n}, Objective {obj_n}: {chunk_text}\n\n"
        ...
        cards.append(card(
            ...
            quality_meta(
                ...
                f"Adds distinct CCCCO source detail chunk {i} for Objective {obj_n}.",
    ...
    challenge = build_challenge(module_n, obj_n, objective_text)
    challenge_text = (
        f"Practice challenge for CCCCO Module {module_n} Objective {obj_n}: {challenge['prompt']}\n\n"
    ...
    cards.append(card(
        ...
        "Practice Challenge", challenge_text,
        quality_meta(module_n, obj_n, objective_text, status, "Checks applied CNA scope judgment for this objective.", "Delivery cards; challenge applies the objective.", source_ref),
        challenge=challenge, key_terms=key_terms[:3],
    ))
    debrief_text = (
        f"Debrief for CCCCO Module {module_n} Objective {obj_n}. The safest response is to use the source concept within CNA scope: "
        "observe the resident, assist only as directed, follow the care plan, report changes, document according to facility policy, and escalate when needed. "
        "This online theory does not validate hands-on competency or create clinical-hour credit."
    )
    cards.append(card(
        f"M{module_n}", module_title, lesson, "C91_DEBRIEF", "debrief",
        "Challenge Debrief / Remediation", debrief_text,
        quality_meta(module_n, obj_n, objective_text, status, "Provides remediation and scope rationale for the practice challenge.", "Challenge card; debrief explains safe reasoning.", source_ref),
...
def build_module(module_obj: dict) -> dict:
    ...
    title = module_obj["title"].replace("Module ", "CCCCO Module ", 1)
    ...
    for objective, mins in zip(module_obj["objectives"], lesson_minutes):
        ...
        lessons.append(build_lesson(module_n, title, objective, mins, terms, section))
    return {
        "module_id": module_id,
        "module_title": title,
        "short_title": title,
        ...
        "sme_review_flag": "None identified",
        "compliance_review_flag": COMPLIANCE_FLAG,
        "source_status_flag": f"Instructional source of truth: CCCCO Module {module_n}.",
        "source_module_number": str(module_n),
        "source_time_basis": "CCCCO Modules 10-17 source-hour weighting normalized to 12-hour recertification theory course.",
        "lessons": lessons,
    }
...
def build_orientation() -> dict:
    ...
    topics = [
        ("L01", "Course Purpose and CCCCO Source Basis", "This course is organized around CCCCO Modules 10 through 17. The orientation explains navigation, scope, privacy, and certificate limitations before instruction begins."),
    ...
    for (lid, title, text), mins in zip(topics, lesson_minutes):
        ...
        meta = quality_meta(0, int(lid[1:]), title, "Orientation", "Provides required course boundary orientation.", "Instructional CCCCO module lessons.", "Orientation/compliance exception")
        challenge = {
        ...
        lesson["cards"] = [
            ...
            card(module_id, module_title, lesson, "C02_01_DELIVERY", "delivery", title, text + "\n\nThis orientation item is the only non-CCCCO instructional exception in the course structure.", meta),
    ...
    return {
        ...
        "source_status_flag": "Orientation/compliance exception; instructional modules use CCCCO Modules 10-17.",
        "lessons": lessons,
    }
...
def build_module_assessment(module: dict) -> dict:
    ...
    if module_id == "M00":
        ...
        for idx, lesson in enumerate(module["lessons"], start=1):
            questions.append(module_assessment_question(0, idx, lesson["lesson_title"], module["module_title"], idx))
            questions[-1]["source_reference"] = source
    else:
    ...
    return {
        ...
        "questions": questions,
        "status": "Draft - CCCCO source-backed" if module_id != "M00" else "Draft - orientation/compliance",
        "sme_review_flag": "None identified",
...
def build_final_assessment(modules: list[dict]) -> dict:
    ...
    for module in modules:
        ...
        for obj_i, objective in enumerate(module["learning_objectives"], start=1):
            ...
            q["source_reference"] = f"CCCCO Module {module_n}, Objective {obj_i}"
            questions.append(q)
            idx += 1
    return {
        ...
        "attempt_size": 25,
        "pass_percent": 80,
        "instructions": "Final assessment draws from CCCCO Modules 10-17. Learner-facing results never reveal the answer key.",
...
def main() -> int:
    ...
    app_copy["moduleAssessment"]["summary"] = "Each CCCCO module knowledge check verifies source-backed theory understanding without exposing answer keys."
    app_copy.setdefault("clinicalHub", {})
    app_copy["clinicalHub"]["warning"] = "Optional clinical support is disabled/non-credit in this CCCCO-source rebuild and never counts toward required theory."
    ...
    data = {
        ...
        "control_facts": {
            ...
            "required_flags": [
                "Instructional content is sourced from CCCCO Modules 10-17; orientation/compliance is the only exception.",
        ...
        "source_manifest": [
            {"source": "Orientation/compliance exception", "usage": "Course navigation, privacy, identity, and certificate-disabled boundaries"},
            *[
                {
                    ...
                    "usage": f"Instructional source for CCCCO Module {n}",
                }
