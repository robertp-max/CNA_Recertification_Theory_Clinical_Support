...
export type RemediationChallenge = {
  ...
  option_rationales_internal?: Record<string, string>;
};
...
export function buildLessonRemediation(challenge: RemediationChallenge): LessonRemediation {
  ...
  const options: OptionReview[] = challenge.choices.map((choice) => {
    ...
    const authoredWhy = challenge.option_rationales_internal?.[choice.id];
    if (isSafest) {
      return {
        id: choice.id,
        label: choice.label,
        status: "safest",
        why: sentence(authored?.why ?? authoredWhy ?? override?.whySafest ?? challenge.rationale_internal, whySafest),
        remember: sentence(
          authored?.remember,
